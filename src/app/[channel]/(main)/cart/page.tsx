...
export const metadata = {
	title: "购物车 · Saleor Storefront 示例",
};

export default function Page(props: { params: Promise<{ channel: string }> }) {
	return (
		<section className="mx-auto max-w-7xl p-8">
			<h1 className="mt-8 text-3xl font-bold text-neutral-900">您的购物车</h1>
...
async function CartContent({ params: paramsPromise }: { params: Promise<{ channel: string }> }) {
	...
	if (!checkout || checkout.lines.length < 1) {
		return (
			<div className="mt-12">
				<p className="my-12 text-sm text-neutral-500">您似乎还没有添加任何商品到购物车。</p>
				<LinkWithChannel
					href="/products"
					...
					<li key={item.id} className="flex py-4">
						...
						<div className="relative flex flex-1 flex-col justify-between p-4 py-2">
							<div className="flex justify-between justify-items-start gap-4">
								<div>
									...
									{item.variant.name !== item.variant.id && Boolean(item.variant.name) && (
										<p className="mt-1 text-sm text-neutral-500">款式: {item.variant.name}</p>
									)}
								</div>
							...
							<div className="flex justify-between">
								<div className="text-sm font-bold">数量: {item.quantity}</div>
			...
			<div className="mt-12">
				<div className="rounded border bg-neutral-50 px-4 py-2">
					<div className="flex items-center justify-between gap-2 py-2">
						<div>
							<p className="font-semibold text-neutral-900">总计</p>
							<p className="mt-1 text-sm text-neutral-500">运费将在下一步计算</p>
						</div>
